...
public:
    ...
    explicit allocator(const allocator<OtherType> &other)
        : alloc_(other.get_alloc()), dealloc_(other.get_dealloc()) {}

    pointer allocate(std::size_t const n) {
        return static_cast<pointer>(alloc_(n * sizeof(value_type)));
    }

    void deallocate(const pointer p, std::size_t n) { dealloc_(p); }
    ...
    const alloc_type &get_alloc() const { return alloc_; }

    const dealloc_type &get_dealloc() const { return dealloc_; }
    ...
    bool operator!=(const_reference other) const { return !this->operator==(other); }

    static std::size_t max_size() { return std::size_t(-1) / sizeof(value_type); }
