...
void EventDispatch::register_io_listener(IoListener &listener, Event event) {
    epoll_event ev;
...
void EventDispatch::run_once(std::chrono::milliseconds timeout) {
    // 处理循环事件
    on_loop();
    ...
    // 处理 IO 事件
    for (int i = 0; i < n; ++i) {
        auto const listener = static_cast<IoListener *>(events[i].data.ptr);
...
void EventDispatch::on_timeout(TimePoint now) {
